...
- 지도에서 현재 위치 버튼을 누르면 브라우저 GPS 좌표를 앱 공용 메모리에 보관한다.
- GPS 사용 중에는 선택 동네로 가맹점을 제한하지 않고 현재 위치 5km 반경을 사용한다.
- 지도 목록, 거리, AI 추천, 챗봇 추천, 홈 가맹점 수, 편의점 목록이 같은 위치 기준을 공유한다.
- GPS 좌표는 서버·로컬 스토리지에 저장하지 않으며 새로고침하면 삭제된다.
- 위치 권한이 없거나 사용자가 지역을 직접 선택하면 기존 동네 중심 좌표를 대체 기준으로 사용한다.
