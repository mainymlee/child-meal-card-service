import type { MealFeedback, MealLogEntry, MenuItem, NutritionGroup, Store } from "./types";
...
export function nutritionFitScore(
  ...
  feedback.slice(-20).forEach((item, index, items) => {
    ...
    if (item.grp === grp) score += item.satisfaction * 0.8 * recency;
    if (item.menuName === menu.name) score += item.satisfaction * 2.2 * recency;
    if (item.reason === "spicy" && current.sodium === "높음") score -= 1.4 * recency;
    if (item.reason === "repeat" && item.grp === grp) score -= 1.2 * recency;
    if (item.reason === "portion" && current.protein === "낮음") score -= 0.8 * recency;
  });
  return score;
}

export function bestNutritionMenu(
  store: Store,
  history: MealLogEntry[],
  feedback: MealFeedback[] = []
): MenuItem | undefined {
  ...
  return [...pool].sort((a, b) =>
    nutritionFitScore(b, store.grp, history, feedback) - nutritionFitScore(a, store.grp, history, feedback) || a.price - b.price
  )[0];
}
